...
- **Libraries** (Sketch → Include Library → Manage Libraries):
  ...
  - **BSEC2** (Bosch Sensortec) — for BME680 compensated T/RH/P and official IAQ on the GUI sketch. Accept Bosch license when prompted. State is saved to NVS (Preferences); no SD card. Note: precompiled library for **ESP32-C6** may be missing in some BSEC2 releases (link error); use a board with supported architecture (e.g. ESP32, ESP32-S3) or fall back to Adafruit BME680 + gas IAQ if needed.
...
- `environmental_monitor_gui/` — Same sensors + LCD (172×320), BME680+BSEC2, BSEC state in NVS, optional WiFi+MQTT; `config.h` has I²C, TFT, and MQTT settings
